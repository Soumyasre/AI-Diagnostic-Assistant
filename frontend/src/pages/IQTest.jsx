...
function IQTest() {
  ...
  const calculateScore = async (userAnswers) => {
    ...
    if (duration < 5) {
      total = 0;
      alert("Too fast! Automatic disqualification.");
    } else if (duration > 780) {
      total *= 0.75;
      alert("Took too long! Score reduced by 25%.");
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Failed to save IQ score: ${data.error}`);
      } else {
        console.log('IQ score saved successfully!');
      }
    } catch (error) {
      console.error('Error during save-score fetch:', error);
      alert('Error connecting to server while saving IQ score.');
    }
